...
| **T5 — Verticales (data)** ✅ AMORCÉ (1er lot) | `templateStructures.extra.ts` : **8 nouvelles verticales en DONNÉES** (Artisan & BTP : plombier/électricien/serrurier ; Beauté : barbier/institut ; Santé : dentiste/ostéo ; Commerce : fleuriste) → registre passe de 20 à **28 structures** (≈ 28 × 14 styles × 3 layouts = **1176 combinaisons**). Avis marqués « (exemple) » (anti-fake). Ajouter un métier = ajouter un objet | +4 tests (types valides, anti-fake, compose) + **captures vérifiées** (plombier/gold, barbier/cocktail = pages premium, 0 erreur) |
