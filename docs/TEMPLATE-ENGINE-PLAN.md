...
| **T5 — Verticales (data)** ✅ 2 lots | `templateStructures.extra.ts` : **18 nouvelles verticales en DONNÉES** — Artisan & BTP (plombier/électricien/serrurier/paysagiste), Beauté (barbier/institut), Santé (dentiste/ostéo), Commerce (fleuriste), Restauration (pizzeria/coffee/boulangerie), Immobilier (chasseur/gîte), Créatif (photographe/DJ), Événementiel (wedding planner), Coaching (coach sportif) → registre **20 → 38 structures** (≈ 38 × 14 styles × 3 layouts = **1596 combinaisons**). Avis « (exemple) » (anti-fake, test garde-fou). Ajouter un métier = ajouter un objet | +4 tests + **captures vérifiées** (plombier/gold, barbier/cocktail, pizzeria/neon, wedding/rose = pages premium, 0 erreur) |
