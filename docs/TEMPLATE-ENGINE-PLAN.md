...
| **T5 — Verticales (data)** ✅ 3 lots | `templateStructures.extra.ts` : **30 nouvelles verticales en DONNÉES** — Artisan/BTP, Beauté (barbier/institut/ongles/tatoueur/spa), Santé (dentiste/ostéo/psy/nutritionniste/vétérinaire), Commerce (fleuriste/bijouterie/cave/animalerie), Restauration (pizzeria/coffee/boulangerie), Immobilier (chasseur/gîte), Créatif (photographe/DJ), Événementiel (wedding), Coaching, Freelance (avocat/auto-école) → registre **20 → 49 structures** (≈ 49 × 14 styles × 3 layouts = **2058 combinaisons**). Galeries via placeholders (T4). Avis « (exemple) » (anti-fake, garde-fou). Ajouter un métier = ajouter un objet | tests garde-fous (clés uniques, types valides, anti-fake) + **captures vérifiées** (plombier, barbier, pizzeria, wedding, photographe, tatoueur = pages premium, 0 erreur) |
