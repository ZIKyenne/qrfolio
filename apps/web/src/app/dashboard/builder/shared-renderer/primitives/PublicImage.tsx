...
export function PublicSharedImage({ model, width, height, style }: { model: SharedImageModel; width: number; height: number; style: CSSProperties }) {
  if (!model.src) return null
  return <SmartImage src={model.src} alt={model.alt} width={width} height={height} style={style} onError={e => { e.currentTarget.style.display = "none" }} />
}
