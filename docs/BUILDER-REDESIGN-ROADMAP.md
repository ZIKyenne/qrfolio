...
| **9 (C09, faite)** | **Finition premium** : grille bibliothèque auto-responsive (fin des cartes tronquées), canvas plus profond + ombre page, insertion `+` élégante (hover), zone dangereuse *ghost*, cartes à élévation, micro-transitions ; tokens `shadow`/`transition`. Vérifié sur captures navigateur. Voir `BUILDER-PREMIUM-FINISHING.md` | builderUi | **navigateur RÉEL (captures avant/après)** ✅ |
| **10 (prochaine)** | Productivité : outline docké + DnD, plus de commandes palette, presets | nav, actions | navigateur |
