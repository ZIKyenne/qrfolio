      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {items.map((sh, i) => (
          <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, background: "rgba(145,70,255,0.06)", border: "1px solid rgba(145,70,255,0.2)", borderRadius: 13, padding: "12px 15px" }}>
            ...
            {sh.link.visible && <a href={sh.link.href || "#"} target="_blank" rel="noopener noreferrer" onClick={() => { try { trackClick(sh.link.trackTarget) } catch {} }} style={{ background: "#9146FF", borderRadius: 8, padding: "7px 13px", fontSize: 12, fontWeight: 700, color: "#fff", flexShrink: 0, textDecoration: "none" }}>Billets →</a>}
          </div>
        ))}
      </div>
    </div>
